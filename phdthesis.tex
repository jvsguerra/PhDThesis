...
The parKVFinder software was applied in two case studies published in scientific journals to investigate proteins of therapeutic interest. These analyses explored the dynamics of the flaps of the \ac{HIV-1} protease \cite{guerra2020} and the hydrophobicity profile of binding sites in alphaviruses \cite{ribeiro2021}. Next, we will describe each of these case studies in detail.
...
Molecular recognition hinges on the accessibility of a ligand to the binding site of its corresponding receptor. The residues exposed to the solvent in a target biomolecule constitute the atoms accessible to potential ligands. For instance, identifying protein-protein interfaces through conventional methods, such as cavity detection, proves challenging due to these interfaces being often large, flat, and featureless---classified as "undruggable". Since \acsp{PPI} typically involve contiguous epitopes of one partner and a well-defined groove or series of specific small pockets \cite{jubb2012}, recognizing exposed residues facilitates a more focused study of interaction hotspots in a target receptor, especially in molecular docking studies (\eg, ligand-protein docking and protein-protein docking). In this scenario, we developed \ac{SERD} \cite{SERD}, an open-source tool, licensed under GPL v3.0, designed to detect solvent-exposed residues of a target biomolecule (Figure \ref{fig:serd-spyke}). The algorithm approximates a solvent or ligand molecule to a spherical probe, scanning the target biomolecule, to identify regions (\ie, residues) accessible to this spherical probe, similar to the 3D grid scanning of the \textit{Probe Out} probe in parKVFinder (Figure \ref{fig:parkvfinder-schema}). In other words, probe size defines a cutoff for the solvent- or ligand-accessible surfaces (Figure \ref{fig:serd-spyke}B), selecting residues beyond this cutoff (Figure \ref{fig:serd-spyke}C).

Graph theory has been relevant in both biology and pharmaceutical sciences, applied in the study of protein structure, function, and evolution, as well as in \acs{MD} analysis and receptor-ligand interaction networks \cite{vishveshwara2002,mason2007,hummer2023}. As presented in Graphinity \cite{hummer2023}, representations of protein-protein complexes (\ie, antibody-antigen complexes) can be employed in deep learning techniques to predict experimental $\Delta \Delta G$. Additionally, graph-based representations can be used in a broad range of machine learning techniques and data science, as illustrated in Section \ref{sec:graph-representation} and discussed in previous studies \cite{majeed2020,vishveshwara2002,mason2007}. In this context, \acs{SERD} also represents exposed residues in graph form using the NetworkX library \cite{networkx}, forming edges up to a specified distance between \acs{CA}, \acs{CB}, or any atoms of the residue, optionally including these distances as attributes of the edges of these graphs, as illustrated in Figure \ref{fig:serd-spyke}D.
...
Beyond its original scope, the graph-based representations developed in SERD are also applicable to binding sites found by pyKVFinder (Figure \ref{fig:serd-cavity}). This topological representation considers intramolecular interactions for constructing the edges of the graphs, providing new ways of analyzing binding sites in biomolecules using graph theory.

\begin{figure}[h]
  \centering
  \includegraphics[scale=0.8]{images/graph-representation-of-binding-sites.png}
...
In summary, SERD can represent biomolecular structures as graphs, from ligand-binding sites (\eg, cavities identified by the KVFinder suite) to biomolecular complexes (\eg, IPPs, IPLs, IPRs, and IPDs), as shown in Figure \ref{fig:graph-representation}. To date, this tool has been applied in two collaborations at the Computational Biology Laboratory: the Master's project of student Marcos Rogério Simões from PPGCF/FCF, entitled "Description and characterization of binding sites through graph theory", studying the similarity between binding sites among kinase families, and the research project of Dr. Gabriel Ernesto Jara, studying antibody-antigen complexes through surface fragments, represented as graphs.  Currently, \acs{SERD} is in version \href{https://github.com/LBC-LNBio/SERD/releases/tag/v0.1.2}{v0.1.2}. The source code is under continuous development and available at the following repository: \url{https://github.com/LBC-LNBio/SERD}.
