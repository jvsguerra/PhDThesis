...
\tikzstyle{box}=[rectangle, draw=black, fill=gray!20, drop shadow, text centered, anchor=north, align=center]
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[node distance=0.5cm]
    ...
    \node (GeomBased) [box, text width=6cm] {\textbf{Geometry-based methods}};
    \node (AuxNode01) [below=of GeomBased] {};
    \node (ProbeBased) [box, text width=3cm, rectangle split, rectangle split parts=2, below left=0.5cm and 4cm of AuxNode01] 
    {
    ...
    \node (GridBased) [box, text width=3.5cm, rectangle split, rectangle split parts=2, below left=0.5cm and 0cm of AuxNode01]
    {
    ...
    \node (SurfBased) [box, text width=3cm, rectangle split, rectangle split parts=2, below right=0.5cm and 0.0cm of AuxNode01]
    {
      \textbf{Surface-based methods}
      \nodepart{two}{NSA \ignorecitefornumbering{\cite{nsa}}\\ MSPocket \ignorecitefornumbering{\cite{mspocket}}}
    };
    \node (TessBased) [box, text width=4cm, rectangle split, rectangle split parts=2, below right=0.5cm and 3.5cm of AuxNode01]
    {
    ...
    \node (AuxNode02) [below left=4.25cm and 6cm of AuxNode01] {};
    \node (AuxNode03) [below left=4.25cm and 2.5cm of AuxNode01] {};
    \node (AuxNode04) [below left=4.25cm and 4.5cm of AuxNode01] {};
    \node (GPBased) [box, rectangle split, rectangle split parts=2, below=0.5cm of AuxNode04, text width=4cm]
    {
    ...
    \node (GSBased) [box, rectangle split, rectangle split parts=2, below=5cm of AuxNode01, text width=4cm]
    {
    ...
    \node (AuxNode05) [below left=4.25cm and 1.25cm of AuxNode01] {};
    \node (AuxNode06) [below right=4.25cm and 0.75cm of AuxNode01] {};
...
Computational problems are typically divided into distinct parts, each comprising sets of instructions. In computing, two fundamental approaches exist: serial computing, which executes instructions sequentially, and parallel computing, which leverages multiple computational resources simultaneously (Figure \ref{fig:computing}). This concurrent execution enables the simultaneous resolution of instructions, thereby reducing computational time---a critical advantage in the context of modern computers  featuring parallel architectures with multiple processors \cite{foster1995,grama2003}.

% Add tikz diagram for serial x parallel computing
% Template:
\begin{figure}
  \centering
  \begin{tikzpicture}[node distance=1.5cm,scale=0.49]
      \node (start) [circle, draw] {Start};
      \node (task1) [rectangle, draw, below of=start] {Task 1};
      \node (task2) [rectangle, draw, below of=task1] {Task 2};
      \node (dots) [below of=task2] {\vdots};
      \node (taskn) [rectangle, draw, below of=dots] {Task n};
      \node (end) [circle, draw, below of=taskn] {End};
      ...
      \draw [->] (taskn) -- (end);
  \end{tikzpicture}
  \hspace{3cm} % Space between diagrams
  \begin{tikzpicture}[node distance=1.5cm,scale=0.49]

    % Define nodes
    \node (start) [circle, draw] {Start};
    \node (task1) [rectangle, draw, below of=start] {Task 1};
    \node (task2) [rectangle, draw, below right=1cm and 2cm of task1] {Task 2};
    \node (task3) [rectangle, draw, below left=1cm and 2cm of task2] {Task 3};
    \node (task4) [rectangle, draw, below right=1cm and 2cm of task3] {Task 4};
    \node (end) [circle, draw, below of=task4] {End};
