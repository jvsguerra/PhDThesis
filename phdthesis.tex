...
% Opens up a new class of mathematical approaches in graph theory

% Review it

Building upon the atomistic representation, the utilization of graph-based models emerges as a robust representation for examining interactions and relationships within biomolecules. This approach, widely applied across diverse scientific disciplines such as biology, chemistry, physics, computer science, and mathematics \cite{foulds1995, majeed2020}, leverages the power and flexibility inherent in graphs as versatile data structures. In structural biology, this representation has been utilized to investigate the structure, folding, stability, function, and dynamics of proteins \cite{vishveshwara2002}.

A graph $\mathcal{G(V,E)}$, is a mathematical structure, that consists of a set $\mathcal{V}$ of vertex (also known nodes) and a set $\mathcal{E}$ of edges connecting these vertices (Figure \ref{fig:graph-examples}). A distinction is made between undirected graphs, where edges link two vertices symmetrically, and directed graphs (also known as digraph), where edges link two vertices asymmetrically. Our simple graph has edges that are unbiased or unweighted. If instead these edges carry relative weight, this graph is known as a weighted graph \cite{}. In biomolecules, which are assemblies of atoms (vertices) connected by intramolecular and intermolecular interactions (edges), they have also been extensively explored through graph theory \cite{vishveshwara2002,mason2007}.

% Paul E. Black and Paul J. Tanenbaum, "graph", in Dictionary of Algorithms and Data Structures [online], Paul E. Black, ed. 18 July 2022. (accessed TODAY) Available from: https://www.nist.gov/dads/HTML/graph.html
