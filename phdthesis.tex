...
%% Add infographic similar to Simões et al. 2017, summarizing classification of geometric approaches, methodologies in each category and their advantages and disadvantages (?)

The detection of cavities through geometric approaches is widespread, encompassing various techniques \cite{simoes2017,guerra2020,krone2016}. These techniques are simple, straightforward, and do not require prior knowledge, making them the most commonly used in the literature \cite{henrich2010,oliveira2014}. In this context, we present a brief classification of geometric approaches for cavity detection, including techniques based on tridimensional (3D) grids, probes, tessellation, surface, and their combinations \cite{simoes2017,guerra2020,krone2016,guerra2023B}.
...
It is noteworthy that the original KVFinder software \cite{oliveira2014}, introduced in 2014, has been deprecated. However, subsequent implementations, namely parKVFinder \cite{guerra2020}, pyKVFinder \cite{guerra2021}, and KVFinder-web \cite{guerra2023A}, have been developed to enhance computational performance and usability. Each of these tools addresses different scientific community demands in a flexible manner. Cavity characterization in these tools includes morphological descriptors (\eg, volume, area, shape, and depth), topological descriptors (\eg, interface residues surrounding the cavities and their classification---aliphatic, aromatic, polar uncharged, negatively charged, and positively charged---), and physicochemical descriptors (\eg, hydrophobicity). Further details on the KVFinder suite can be found in Chapter \ref{ch:kvfinder-suite}.

\subsection{Fpocket}

Fpocket \cite{fpocket} is a tessellation-based tool that performs pocket detection based on the concept of \textalpha-spheres, introduced by \cite{liang1998}. The cavity detection algorithm (Figure \ref{fig:fpocket-schema}) determines the set of \textalpha-spheres from the target structure, using the \textit{qhull} package, and removes spheres outside a minimum and maximum radius size. Cavities are clusters of \textalpha-spheres, formed based on proximity and neighborhood relationships, with uninteresting cavities removed from further analysis. The remaining cavities are assessed using a set of dpocket descriptors \cite{fpocket} and classified according to their presumed binding capacity to a small molecule.

\subsection{GHECOM}

\subsection{CAVER tools}
...
% \begin{figure}[ht]
%   \centerline{\includegraphics[scale=1]{images/kvfinder-suite-schema.png}}
%   \centerline{\tiny{\textbf{Fonte:} Retirado de \cite{guerra2023B}.}}
...
%   \centerline{\tiny{\textbf{Fonte:} Retirado de \cite{guerra2021}.}}
