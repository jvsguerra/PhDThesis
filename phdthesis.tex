...
\subsection{Computational complexity \label{sec:computational-complexity}}
...
\begin{figure}[H]
  ...
  \centerline{\tiny{\textbf{Source:} Reprinted with permission from \cite{guerra2023B}. Copyright 2023 American Chemical Society.}}
  \caption[Channel and tunnel detection algorithm in CAVER 3.0 and CAVER Analyst 2.0]{\textbf{Channel and tunnel detection algorithm in CAVER 3.0 and CAVER Analyst 2.0.} \textbf{(A)} A molecular shape is probed by a spherical probe, called the \textit{shell probe}, with a radius specified by the \textit{shell radius} parameter, to define an outer surface SAS (red line). From it, a distance specified by the \textit{shell depth} parameter is removed to define an inner surface (yellow line). \textbf{(B)} A pseudo-Voronoi diagram is constructed based on the molecular shape. Voronoi vertices (orange points) are used to create the central lines of the tunnel/channel. \textbf{(C)} A starting point (red point) is a user-defined parameter set as the center of mass of the molecular shape, and an endpoint (blue point) is set at the center of the inner surface. From the starting point, the central line passes through Voronoi edges and vertices to form a tunnel/channel to the outer surface and passes through the endpoint. \textbf{(D)} Spheres are fitted at all points of the central line, from the starting point to the endpoint, defining the bottleneck (mouth opening) radius along the tunnel and/or channel.}
  \label{fig:caver-schema}
\end{figure}

\section{Parallel Computing \label{sec:parallel-computing}}

Parallel computing is a paradigm that revolutionizes traditional sequential computing by harnessing the power of multiple processors or computing units to perform tasks concurrently. This approach is driven by the need for enhanced computational speed and efficiency, especially as we confront escalating data volumes and complex problem-solving requirements. 

The roots of parallel computing trace back to the early days of computing, but it gained prominence as computational demands surged. Early parallel systems were tightly coupled, sharing memory and closely coordinating tasks. The evolution of parallel computing architectures progressed from SIMD (Single Instruction, Multiple Data) to MIMD (Multiple Instruction, Multiple Data) systems, marking a shift towards greater flexibility and efficiency.

Parallel computing architectures encompass diverse designs tailored to specific computational needs. Shared-memory architectures, such as Symmetric Multiprocessing (SMP), enable processors to access a common memory pool. Distributed-memory architectures, on the other hand, like those in clusters or grids, involve processors with their dedicated memory. Hybrid architectures combine these models, leveraging the strengths of both shared and distributed memory systems.

While parallel computing offers immense potential, it introduces challenges that demand careful consideration. Coordinating parallel tasks, managing data synchronization, and minimizing communication overhead are critical concerns. Load balancing becomes pivotal to ensure uniform utilization of resources, preventing bottlenecks that can hinder performance gains. Additionally, designing algorithms that effectively parallelize tasks is a complex endeavor, requiring a deep understanding of both the problem domain and the intricacies of parallel architectures.

Parallel computing finds applications across diverse domains, from scientific simulations and data analytics to artificial intelligence and beyond. High-performance computing (HPC) clusters tackle intricate scientific problems, while parallel processing accelerates the training of machine learning models. Parallel algorithms play a crucial role in speeding up tasks like sorting, searching, and graph traversal, contributing to improved efficiency in various computational domains.

The future of parallel computing is shaped by ongoing advancements in hardware, software, and algorithms. Emerging technologies such as quantum computing introduce new dimensions to parallelism. Exploring novel ways to harness parallelism efficiently, overcoming scalability challenges, and integrating parallel computing into mainstream applications are among the ongoing research directions. In summary, parallel computing stands as a cornerstone in addressing the escalating demands for computational power. 
...
% Discuss time complexity and spatial complexity of parKVFinder
% Discuss bit-depth for int grid, char grid and double grid

% In general scenario, the cavity detection algorithms scale linearly with the input size. For grid-based approaches and their hybrid counterparts, the algorithms that scale linearly with the number of voxels are considered efficient, while those that scale exponentially are considered inefficient. Every grid-based method is sensitive to grid spacing. Higher density grids enable richer spatial representation, which are useful for detailed study of cavities, but imply higher computational costs. In fact, a grid with smaller voxels implies more memory space consumption and poorer time performance. For tesselation-based approaches, usually the algorithms scale linearly with the number of atoms, which is the input size. 

% In this context, we present a brief analysis of the computational complexity of the parKVFinder algorithm. Specifically, for parKVFiner software, the voxel length is a user-defined parameter, and the default value is 0.6 Å. For this class of algorithm, grid spacing oscillates from 0.5 to 0.8 Å. However, the user can set a different value, depending on the application. For instance, if the user is interested in more accuracte morphological characterization of the biomolecular cavity, a smaller voxel length is recommended, such as 0.25 Å. The complexity of the problem is intricately linked to the number of voxels, denoted as O(n), where n represents the number of voxels. No entanto, vale ressaltar que o tamanho médio da grade tridimensional é aproximadamente 150x150x150. The challenge intensifies as the region of interest, dictated by the probe size, influences the voxel count. The inherent nature of the algorithm, focused on voxel-level analysis, amplifies computational demands. 

% In a general scenario, the time spatial complexity is O(n). The most expansive step being the voxel clustering algorithm, that is a recursive implementation of the DFS algorithm. The bit-depth of grid elements is a significant factor influencing memory usage and precision. In C language, the choice between int (32-bit integer) and double (64-bit floating-point) is pivotal. This choice balances memory efficiency with the precision required for accurate calculations. Implemented in C, ParKVFinder adopts a nuanced approach by utilizing both int and double for grid elements. This choice reflects a careful consideration of memory efficiency and precision requirements. The decision to use int or double hinges on the algorithm's needs and the characteristics of the processed data.
