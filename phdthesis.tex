...
\acs{MD} analysis tools face challenges to define the continuity of cavities throughout the simulation. From the \acs{MD} simulation of the \acs{HIV-1} protease, in which we successfully described the conformational dynamics of the active site (see Section \ref{sec:md-hiv1-protease}), we developed a structural cavity alignment methodology to define the cavity similarity throughout \acs{MD} simulation, using KVFinderMD. In simpler terms, we applied the KVFinderMD to detect cavities in the \acs{HIV-1} protease throughout the \acs{MD} simulation. Then, using different cavity representations available (\ie, 3D grid representation, residue-level representation, and graph-based representation), an unsupervised clustering algorithm group cavities (\ie, binding sites), based on the similarity of their cavity representation, exploring different affinity metrics.

First of all, each cavity was separated into an individual data structure (Figure \ref{fig:hiv-1-representation}). For the 3D grid representation, cavities are stored as 3D grid of booleans, marking cavity points with 1, while other points (biomolecule points, solvent points, and empty space points) received 0. For the residue-level representation, cavities are stored as square matrix of distances between residues forming them, ordered according to the protein sequence. For the graph-based representation, cavities are stored as a square matrix of contacts between residues forming them, also ordered according to the protein sequence. The contacts are defined by the distance between the \acs{CB} atoms of residues, with a distance cutoff of 8 $\AA$ to define contacts, the default metric of KVFinderMD.

\begin{figure}[h]
  \centering
  \includegraphics[scale=1]{images/HIV-1-representation.png}
  \caption[Data coding for the study of HIV-1 protease cavity similarity throughtout molecular dynamics trajectory]{\textbf{Data coding for the study of HIV-1 protease cavity similarity throughtout molecular dynamics trajectory.} Representation of the active site cavity in 3D grid (left panel), where cavity points are identified by 1, while other points (biomolecule points, solvent points, and empty space points) are identified by the value 0. Residue-level representation of the active site (central panel), where we abstract cavities as a distance matrix between \acs{CB} atoms of residues, ordered by the protein sequence. Graph-based representation of relationships between active site residues (right panel), where we abstract cavities as a contact matrix, ordered by the protein sequence.}
  \label{fig:hiv-1-representation}
\end{figure}

Then, clustering analysis can group cavities (\ie, cavity representations) based on their similarity. Hierarchical clustering, compared to other unsupervised clustering methods, yield more interpretable results, by providing a similarity relationship between cavity representations. As there is no ground truth for an unlabeled cluster analysis (\ie, structural cavity alignment), the silhouette score ($s$; Eq. \ref{eq:silhouette}), proposed by Peter Rousseeuw \cite{rousseeuw1987}, was used to evaluate the quality of the clustering. This metric measures the similarity of a cavity to its own group (cohesion) compared to other groups (separation), based on an affinity metric. Thus, achieving a silhouette score of 1 is optimal for cluster validity, while a score of -1 signifies poor clustering, and 0 implies clusters with overlapping boundaries.
...
\noindent where $\tilde{s}(k)$ represents the mean $s(i)$ over all cavities of the \acs{DM} simulation for a specific number of clusters $k$.
...
Thus, we applied the unsupervised hierarchical clustering algorithm, optimizing the silhouette coefficient for each affinity metric, using the complete linkage method (Figure \ref{fig:hiv-1-clustering-results}). For the 3D grid representation, we did not explore different affinity metrics due to data size ($\sim$$10^6$ points per cavity), and therefore, we used correlation distance (Eq. \ref{eq:correlation}), where we found 10 groups with a maximum silhouette coefficient of $\sim$$0.42$. With data size simplification for the remaining two representations ($\sim$$10^4$ points per cavity), it was possible to explore appropriate affinity metrics. For the residue-level representation, cosine distance (Eq. \ref{eq:cosine}) showed the highest number of groups among non-negative real-valued metrics (\ie, 15 groups) with a maximum silhouette coefficient of $\sim$0.53. For the graph representation, Dice dissimilarity (Equation \ref{eq:dice}) showed the highest number of groups among non-negative real-valued metrics (\ie, 16 groups) with a maximum silhouette coefficient of $\sim$0.52.
...
From these clusters, we successfully grouped cavities and tracked their evolution throughout the \acs{MD} simulation. This simplified the comparison of characteristics between cavities over time. The use of distance matrices and contact matrices proved effective due to their ability to cluster cavities. Moreover, these representations were faster and simpler for clustering cavities over time in MD simulations since the hierarchical clustering algorithm has a complexity of approximately $\mathcal{O}(n^3)$, and the data size decreases by 100 times per cavity compared to the original structure. Therefore, considering we have 672 cavities in this case study, the reduction in memory usage is approximately 67,200 times.
