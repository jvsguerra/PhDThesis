...
\begin{abstract}
%PSLO AGORA CORRIJA NO ABSTRACT TB
...
These interactions involve receptors and ligands, spanning ions to macromolecules such as proteins, RNA, and DNA \cite{oliveira2014}. Molecular structures intricately
%PSLO ESSE INTRICATE ESTÁ MUITO REPETITIVO NO TEXTO
...
\begin{itemize}
  \item \textbf{Grid-based algorithms} (\eg, CavitySearch \cite{cavitysearch}, POCKET \cite{pocket}, LIGSITE \cite{ligsite}, POVME 3.0 \cite{povme}) represent a set of atoms as discrete points, usually using a \acs{3D} grid aligned to the axes as a \comment{scalar field}{o que vc quer dizer: scalar var type? }, \ie, a density map, where each discrete point is an integer or boolean value. These grid maps are used to cluster \remove{relevant} empty \remove{space} points (\ie, not belonging to the solute) into cavities using voxel clustering algorithms. Typically, these methods use simple data structures capable of representing a collection of data at a discrete point and identifying cavities automatically. However, geometric accuracy, computation time, and memory consumption heavily depend on the grid resolution, \ie, grid-spacing sensitivity. Additionally, these methods are not rotation invariant, meaning that the orientation of a given molecule slightly affects accuracy\remove{, \ie, orientation sensitivity};
...
\chapter{Data Encoding in Structural Biology}\comment{Sei que a tradu��o de coding e encoding � a mesma em portugues, mas me soa que o termo coding � mais usado em programa��o para o sentido de criar o codigo de programa��o e encoding seria mais para criar uma codifica��o entre dois sistemas de simbolos? Mas n�o tenho certeza}{comment}

Data encoding, in essence, refers to the process of representing information or data using a specific set of symbols, codes, or conventions. This encoding allows for the efficient storage, transmission, and processing of data. In computer science, data coding can involve various techniques, such as character encoding (representing characters with numerical values), binary coding (using combinations of 0s and 1s to represent information), and other methods tailored to specific types of data.
...
%PSLO aqui você poderoa explorar um pouco mais: quando eu pensei nessa extoria dos grafos eu pensavea em encontar relações entre os  residuos participantes das cavidades que não fossem obvias do espaço euclidiano, por exemplo triades cataliticas ou clusters de residuos hidrofobicos, etc. Secundariamente eu estava começando a estudar GNN e via nessa representação uma maneira de criar coisa s novas em DL 
...
%PSLO João achei que colocar um outro exemplo de HIV-protease no corpo principal da tese  ficaria um pouco redundante. Claro que no paper é outra estoria. Acho que podemos conversar mas se não tiver uma informção realmente nova eu passaria para os suplementar 
...
% Melhor explicar que CEC é o congresso dos estudantes do CNPEM
...
%PSLO João cite explicitamente aqui os artigos. É importante destacar esse resulatado
