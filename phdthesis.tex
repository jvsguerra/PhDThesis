...
\subsubsection{Molecular Volume Estimation}
...
The interface provides the key functionalities of the KVFinder-web service, allowing users to upload a target biomolecule from a PDB file or provide the corresponding PDB code, and customize cavity detection parameters and execution modes (Figure \ref{fig:kvweb-interface}B). Four cavity detection modes are available, offering options that best suit users' cavity analysis needs:

\begin{itemize}
  \item \textit{Whole structure (default)}: uses preset parameters to detect cavities across the whole biomolecular structure;
  \item \textit{Whole structure (customized)}: allows users to customize the detection parameters for cavity detection across the whole biomolecular structure;
  \item \textit{Around target molecule}: detects cavities around a chosen target molecule, focusing the search on the region surrounding that molecule, with customizable detection parameters;
  \item \textit{Around target residues}: detects cavities within a custom box, defined by selecting specific residues within the target biomolecular structure, with customizable detection parameters.
\end{itemize}
...
Additionally, the graphical interface allows users to download and visualize results easily and interactively (Figure \ref{fig:kvweb-results}). Morphological (\ie, volume, area, and depth) and physicochemical (\ie, hydrophobicity) characterizations of each cavity are displayed in an interactive table, available for download in TOML format. A biomolecule viewer, powered by the graphical engine NGL for R (NGLVieweR \cite{nglviewerr}), displays the biomolecular structure with its cavities, available for download in PDB format, and allows various customizations, such as highlighting cavities and displaying interface residues around them.
...
The KVFinder-web service, written in Rust, comprises three modules: the \textit{Web server}, the \textit{Queue}, and the \textit{Worker}. The \textit{Web server} module, employing the Actix web framework (\url{https://actix.rs}), receives job requests via HTTP POST with a JSON-based configuration file containing molecular structures and parKVFinder detection parameters. The molecular structures of a target receptor and optionally a target ligand are stored in \textit{pdb} and \textit{pdb\_ligand} attributes of the JSON, respectively, as PDB-formatted strings. Valid requests prompt the \textit{Web server} to return a response with a unique identifier, created using a hash function based on the incoming JSON data. In case of an invalid request, an HTTP error code with an error message is returned. The job ID creation through a hash function ensures that cached results are retrieved upon resenting a request.
...
The monitoring and analytics of KVFinder-web are essential to ensure the proper functioning of the web application and the quality of the user experience. This process provides valuable insights into how users interact with the application, allowing maintainers and developers to plan improvements and updated tailored to our users requirements. To this end, we implemented a monitoring process that tracks job submission over time and user behavior. 

Tracking the job submission provides valuable information about system usage patterns and helps identify periods of high demand. This insight enables the optimization of resources to ensure a smooth and efficient user experience. Additionally, it helps us identify user adherence to our platform and the impact of new features and updates. From March 2023 to November 2023, KVFinder-web executed 1,039 jobs, with an average of $\sim$115 jobs per month (Figure \ref{fig:job-execution}). From March 2023 to May 2023, KVFinder-web was under development and only accessible for internal users from CNPEM, reaching an average of $\sim$65 jobs per month. In June 2023, KVFinder-web was released to the public, alongside its publication in \textit{Nucleic Acids Research} \cite{guerra2023A}. After publication, KVFinder-web had an average of $\sim$141 jobs per month, showing the user adherance to our web application and the impact of its publication.

\begin{figure}[h]
  \centering
  \includegraphics[scale=0.5]{images/clarityjobs-executed-per-month.png}
...
Understanding user behavior through analytics is crucial for developing KVFinder-web, guiding iterative improvements, optimizing interface design, identifying popular features, and ensuring a smooth and efficient user experience. To perform user behavior analytics, the Cloudflare service (\url{https://www.cloudflare.com}) was initially explored for this purpose, but the collected data did not provide the desired insights, due to Cloudflare having a broader focus, offering services related to content delivery, security, and performance. Its monitoring capabilities are more centered around overall website health and security. From 21 October to 21 November, Cloudflare counted 1,130 visitors, mainly from United States, Netherlands, Germany, China, Spain and Portugal. Subsequently, Microsoft Clarity (\url{https://clarity.microsoft.com}) emerged as a more suitable solution for monitoring user behavior within the KVFinder-web platform. Microsoft Clarity is is a privacy-focused solution that does not collect any personal information, such as IP addresses or cookies. Leveraging Microsoft Clarity allows for a deeper analysis of user interactions, providing valuable insights into how individuals navigate and engage with the application, \ie, presenting click tracking, heatmaps of user engagement, masked session recordings and engagement metrics. Since its implementation, from 21 November to 1 December, KVFinder-web received 49 visitors (Figure \ref{fig:clarity-analytics}), with 28 unique users. The average active time spent in a session was 7.7 minutes out of 12.8 minutes in total, and the scroll depth is 92.57\%. Since its implementation, Microsoft Clarity has provided valuable insights into user behavior, that can be used to refine the user experience and address potential pain points. Therefore, the data collected so far indicates that KVFinder-web is helping to democratize parKVFinder in the scientific community, with users from all over the world, with an an average of $\sim$141 jobs per month since official release.

\begin{figure}[h]
  \centering
  % \includegraphics[scale=1]{images/clarity-analytics.png}
...
Finally, a crucial consideration in this monitoring process is data privacy. Recognizing the sensitivity of user data, stringent measures have been implemented to uphold privacy standards. Access to collected data is restricted exclusively to the individual user through their unique job ID. This ensures that anyone in the maintainer and developer teams has access to the structures submitted to KVFinder-web.
...
% \subsubsection{Caracterização do sítio catalítico da protease do HIV-1}
...
% \subsubsection{Comparação morfológica do sítio catalítico das estruturas da protease do HIV-1}
