...
A codificação de biomoléculas e sítios de ligação desempenha papel fundamental na biologia estrutural, permitindo a representação e análise computacional de informações biológicas complexas. O processo de codificação converte informações biológicas em formatos compreensíveis e adequados para processamento por algoritmos e programas em sistemas computacionais. Em linhas gerais, esse processo  atribui valores ou categorias a componentes biológicos, \eg, átomos, aminoácidos e bases nitrogenadas, para descrever ocupância, localização, características químicas e/ou propriedades estruturais. 

Nesse contexto, a codificação de dados na biologia estrutural é importante para realizar análises avançadas, como modelagem molecular, simulações de dinâmica molecular e predição de interações. Para ilustrar a codificação de dados biológicas para análises computacionais, citamos alguns exemplos de aplicações. Em modelagem molecular, uma proteína pode ser codificada usando códigos de uma letra para representar os diferentes aminoácidos, como ocorre na representação de sequência de proteínas, para predizer o dobramento de proteínas, como aplicado no ESMFold \cite{lin2022}. Em simulações de dinâmica molecular, as coordenadas 3D de átomos ou resíduos, aliados a vetores representando forças ou outros atributos, são utilizados para representar a estrutura de uma biomolécula, como no GROMACS \cite{abraham2015} em simulações atomísticas (\textit{fine-grained molecular dynamics simulation}) e no CafeMol em simulações \textit{coarse-grained} \cite{kenkazi2011}.

Além disso, os sítios de ligação, após identificados por algoritmos computacionais, são codificados de diferentes maneiras para análises posteriores. Os sítios de ligação podem codificados para representar as características físico-químicas e estruturais de uma região de uma biomolécula, como as grades 3D no KVFinder suite \cite{oliveira2014,guerra2020,guerra2021,guerra2023B}, ou as coordenadas 3D dos átomos no fpocket \cite{fpocket}.
...
\section{Representação em grade 3D}

\subsection{Representação de superfícies moleculares}

Ocupância e características.

\begin{itemize}
  \item Superfície de van der Waals;
  \item Superfície acessível ao solvente;
  \item Superfície excluída pelo solvente. 
\end{itemize}
...
Ácidos núcleicos, resíduos e/ou átomos.

\section{Representação em grafos}

Relação entre ácidos núcleicos, resíduos e/ou átomos.
...
% Composto por cinco ferramentas principais, o KVFinder Suite oferece uma ampla gama de funcionalidades para auxiliar na análise estrutural e no estudo de interações biomoleculares.
