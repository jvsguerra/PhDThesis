...
\usepackage[num,abnt-etal-text=emph]{abntex2cite}
\usepackage{cite}
...
\textbf{Parallel KVFinder (parKVFinder)} \cite{guerra2020} é uma ferramenta de código aberto (GPL v3.0) projetado para detecção e caracterização de qualquer tipo de cavidade biomolecular. Ele está disponível com um plugin gráfico para o visualizador molecular PyMOL, chamado \textit{PyMOL2 parKVFinder Tools}, de fácil utilização, que possui uma interface gráfica intuitiva, permitindo aos usuários explorar parâmetros personalizáveis para detecção e caracterização de cavidades. O plugin também permite visualizar as cavidades e suas caracterizações no PyMOL. O código-fonte e o plugin estão disponíveis em https://github.com/LBC-LNBio/parKVFinder.

% Também possui interface de linha de comando (CLI; command-line interface) for advanced users.

O parKVFinder foi originalmente desenvolvido na dissertação de mestrado "Prospecção e caracterização de cavidades supramoleculares" \cite{guerra2019}, como uma versão otimizada e atualizada do KVFinder \cite{oliveira2014}. 

% Posteriormente, a ferramenta foi refatorada e foi publicado na BMC Bioinformatics \cite{guerra2020} e em seguida, novamente otimizado para o KVFinder-web \cite{guerra2023A}.

% parKVFinder is available with an easy-to-use PyMOL plugin with an intuitive graphical user interface that allows users to explore customizable parameters for cavity detection and characterization. In it, cavity characterization (shape, volume, area, and surrounding residues) is performed in each detected cavity. The plugin also allows the user to visualize the cavities and their characterization in the PyMOL viewer. The source code and plugin are available at https://github.com/LBC-LNBio/parKVFinder.

% we developed parKVFinder, a highly versatile and easy-to-use parallelized software for cavity detection and characterization. parKVFinder provides accurate, fast and efficient steered detection and spatial characterization of biomolecular cavities, with a set of intuitive and customizable parameters, which users can interact via a graphical user interface (GUI) or a command-line interface.
