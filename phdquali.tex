...
Nesse estudo, investigamos a dinâmica molecular da HIV-1 protease por meio da identificação e avaliação do volume do sítio ativo durante simulações de dinâmica molecular (DM). O sítio ativo é um alvo terapêutico relevante e seu volume varia de acordo com a movimentação dos \textbeta-hairpins, conhecidos como \textit{flaps}, que controlam a acessibilidade do substrato ao sítio ativo do homodímero \cite{soares2016}. 

O objetivo foi descrever a dinâmica de movimentação dos \textit{flaps} através do volume e forma da cavidade do sítio ativo como descritores conformacionais durante as simulações de MD. Para isso, simulações de DM foram realizadas por 200 ns, usando o pacote GROMACS 2019.4 \cite{gromacs} com o campo de força AMBER99SB-ws e o modelo de água TIP42005s, partindo da estrutura cristalográfica da protease do HIV-1 na conformação fechada \cite{lam1994}, sem o inibidor.

O volume do sítio ativo foi acompanhado ao longo das simulações (Figura \ref{fig:hiv1-protease-dm-analysis}). Observou-se que o volume da cavidade começa no valor correspondente à conformação fechada e, após cerca de 25 ns, começa a aumentar até atingir o valor correspondente à conformação semi-aberta em cerca de 75 ns, indicando um processo de abertura ao longo da simulação. Após esse ponto, os \textit{flaps} se afastam ainda mais, e a cavidade atinge seu volume máximo em cerca de 175 ns antes de reverter para a conformação semi-aberta, que é mais estável. Essa dinâmica de volume correlaciona-se com o desvio médio quadrático (RMSD; \texit{Root-mean-square deviation}, em inglês) do C-\textalpha\space calculado a partir da conformação estável como estado de referência, indicando uma descrição adequada do estado conformacional da proteína ao longo da simulação. É importante destacar que o RMSD é uma métrica que descreve variações globais na estrutura, enquanto o volume estimado é uma métrica direta das mudanças conformacionais do sítio ativo, possivelmente relacionadas à acessibilidade do ligante.
...
% Essa comparação foi realizada identificando e avaliando o vo-
% lume da cavidade que forma o sítio ativo da protease do HIV-1 durante simulações
% de MD. A cavidade que forma o sítio é um alvo terapêutico bastante relevante e seu
% volume varia de acordo com a movimentação dos β-hairpins, denominados flaps, os
% quais controlam a acessibilidade do substrato ao sítio ativo do homodímero (Soares
% et al., 2016). Estruturas cristalográficas apresentam estes flaps em suas conformações
% fechada (Lam et al., 1994) e semi-aberta (Spinelli et al., 1991), enquanto simulações
% de DM investigam a dinâmica de abertura dos flaps (Hornak et al., 2006; Soares et
% al., 2016). O nosso objetivo foi, portanto, descrever a dinâmica de movimentação
% dos flaps através do volume e forma da cavidade do sítio ativo como descritores
% conformacionais durante simulações de DM partindo do estado fechado.
% As simulações de DM (200 ns) foram realizadas com o pacote GROMACS 2019.4
% 8(Spoel et al., 2005), usando o campo de força AMBER99SB-ws com o modelo de
% água TIP42005s (Best, Zheng e Mittal, 2014), partindo da estrutura cristalográfica da
% protease do HIV-1 na conformação fechada (Lam et al., 1994) sem o inibidor. Esses
% dados também estão disponíveis no manuscrito submetido a SoftwareX (ANEXO) ,
% assim como os resultados, que serão descritos a seguir.

% O volume da cavidade do sítio ativo começa no volume correspondente à con-
% formação fechada. Após ~25 ns, o volume começa a aumentar até que em ~75 ns
% atinge o volume correspondente à conformação semi-aberta, indicando um pro-
% cesso de abertura no decorrer da simulação. Após esta marca, os flaps ficam ainda
% mais distantes, e a cavidade atinge seu volume máximo em ~175 ns até reverter à
% conformação semi-aberta, sendo esta mais estável. Esta dinâmica do volume (Fi-
% gura 4) se correlaciona (correlação de Pearson; ρ = 0, 72) com o (RMSD; root-mean-
% square deviation) do Cα calculado a partir da conformação estável como estado de
% referência, indicando a correta descrição do estado conformacional da proteína ao
% longo da simulação. Vale ressaltar que o RMSD é uma métrica que descreve va-
% riações globais em uma estrutura, já o volume estimado é uma métrica direta das
% mudanças conformacionais do sítio ativo, possivelmente relacionados a acessibili-
% dade do ligante.

% As simulações de DM (200 ns) foram realizadas com o pacote GROMACS 2019.4 (Spoel et al., 2005), usando o campo de força AMBER99SB-ws com o modelo de
% água TIP42005s (Best, Zheng e Mittal, 2014), partindo da estrutura cristalográfica da
% protease do HIV-1 na conformação fechada (Lam et al., 1994) sem o inibidor. Esses
% dados também estão disponíveis no manuscrito submetido a SoftwareX (ANEXO) ,
% assim como os resultados, que serão descritos a seguir.
...
% Os resultados e desempenho do parKVFinder foram comparados com outros
% programas baseados em geometria (GHECOM (Kawabata, 2009), Fpocket (Guil-
% loux, Schmidtke e Tuffery, 2009), MSPocket (Zhu e Pisabarro, 2010) e CavVis (Si-
% mões e Gomes, 2019)). Primeiramente, avaliamos como o volume estimado por
% cada programa se correlaciona com o RMSD, como realizamos para o parKVFinder.
% O volume estimado pelo GHECOM também se correlaciona ao estado conformaci-
% onal como o parKVFinder (ρ = 0, 75; Figura 4), possivelmente por ambos empre-
% garem métodos baseados em grade e esfera. Porém, cavidades encontradas pelos
% programas Fpocket (ρ0, 35), MSPocket (ρ = −0, 24) e CavVis (ρ = 0, 19) não se cor-
% relacionam satisfatoriamente com a dinâmica conformacional do sítio ativo. Desta
% forma, parKVFinder e GHECOM apresentaram uma alta acurácia na descrição do
% estado conformacional do sítio ativo da protease do HIV-1.
% Além de boa acurácia, os programas de detecção de cavidades também devem
% ser capazes de realizar detecções e caracterizações rápidas; sendo assim, também
% avaliamos o tempo computacional dos programas. Entre os métodos que apresen-
% taram boa acurácia, o parKVFinder (t = 1h03m) teve um desempenho pelo menos
% quatro vezes mais rápido do que o GHECOM (t = 4h32m e, apesar de ambos se-
% rem baseados no mesmo método, as subrotinas de múltiplas threads do primeiro
% melhoram drasticamente o seu desempenho. Além disso, o parKVFinder também
% superou o MSPocket (t = 2h48m) e o CavVis (t = 3h45), mas não superou o Fpoc-
% ket (t = 20m), o qual utiliza um método baseado em tesselação de Voroni e esferas
% alfa, que é rapidamente computado, porém esta implementação do método se mos-
% trou menos sensível para para descrever finamente as cavidades da protease do
% HIV-1. Apesar deste último ser rápido, o mesmo falhou em diferenciar os estados
% conformacionais do sítio ativo da protease do HIV-1. Portanto, unindo a acurácia e
% o desempenho, o parKVFinder superou os métodos de benchmarking neste caso de
% estudo da protease do HIV-1, ilustrando a robustez dos procedimentos paralelos de
% detecção e caracterização espacial.
