...
\subsubsection{Dinâmica molecular da HIV-1 protease \label{sec:md-hiv1-protease}}
...
\begin{figure}[h]
  \centering
  \includegraphics[scale=1.2]{images/HIV-1-clustering-results.png}
...
\begin{equation}
  Correla\'c\~ao = 1 - \frac{(u - \bar{u}) \cdot (v - \bar{v})}{{\|(u - \bar{u})\|}_2 {\|(v - \bar{v})\|}_2}
  \label{eq:correlacao}
\end{equation}

\begin{equation}
  Cosseno = 1 - \frac{u \cdot v}{\|u\|_2 \|v\|_2}.
  \label{eq:cosseno}
\end{equation}

\begin{equation}
  Dice = \frac{c_{TF} + c_{FT}}{2c_{TT} + c_{FT} + c_{TF}}
 \label{eq:dice}
\end{equation}
...
%  KVFinderMD
%  A ferramenta KVFinderMD se mostrou uma abordagem eficaz para explorar a dinâmica dos sítios de ligação em estruturas biomoleculares de interesse farmacológico. A análise da similaridade das cavidades da protease do HIV-1 ao longo de uma simulação de dinâmica molecular permitiu acompanhar a evolução das cavidades e comparar suas características ao longo do tempo. As representações em grade 3D, representação topológica e representação em grafo foram capazes de descrever as cavidades de forma precisa e eficiente. O uso de algoritmos de agrupamento hierárquico e diferentes métricas de distância contribuiu para a identificação dos grupos de cavidades mais similares. Além disso, a redução do tamanho dos dados e a simplificação da comparação entre as cavidades demonstraram a aplicabilidade prática do KVFinderMD. Essa ferramenta possui potencial para auxiliar na compreensão dos mecanismos de reconhecimento molecular e contribuir para o desenvolvimento de novas terapias farmacológicas.
...
% Nesta seção, apresentamos os casos de estudo aplicando as ferramentas do KVFinder suite: parKVFinder, pyKVFinder, KVFinder-web e KVFinderMD.
